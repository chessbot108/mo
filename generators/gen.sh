...
dense_trial_count=1
dense_trial=1
g++ -std=c++20 -O2 dense.cpp -o dense
while [ $dense_trial -le $dense_trial_count ]
do
	./dense $N $D $trial > ${trial}.in
	echo "${trial} test printed. use dense as generator"
	trial=$((trial+1))
	dense_trial=$((dense_trial+1))
done
rm dense
